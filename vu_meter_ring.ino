...
void loop() 
{
  ...
  for (int i=0;i<=numPixelsring-1;i++){
    strip.setPixelColor(j,Wheel(map(i,0,numPixelsring-1,30,150)));
  }
  ...
  if (c <= numPixelsring) { // Fill partial column with off pixels
    drawLine(numPixelsring, numPixelsring-c, strip.Color(0, 0, 0));
  }
  ...
  y = numPixelsring - peak;
  strip.setPixelColor(y-1,Wheel(map(y,0,numPixelsring-1,30,150)));
  strip.show();
